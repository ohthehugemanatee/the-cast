...
/**
 * @file
 * Theme settings for the f1ux theme.
 */

/**
 * Implements hook_form_system_theme_settings_alter().
 */
function f1ux_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  $recommended_modules = f1ux_recommended_modules();
  ...
  if (!empty($recommended_modules)) {
    ...
    $form['recommended_modules']['hide_recommended_modules'] = array(
      ...
      '#ajax' => array(
        'callback' => 'f1ux_ajax_settings_save',
      ),
      '#default_value' => $hide,
    );
 ...
 */
function f1ux_ajax_settings_save($form, $form_state) {
  $theme = isset($form_state['build_info']['args'][0]) ? $form_state['build_info']['args'][0] : '';
 ...
 */
function f1ux_recommended_modules() {
  $modules = array();
